	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
	      $cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
 		  if(isset($sheet['cells'][$x][$y]) && $sheet['cells'][$x][2] == "Dietistica"){
	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
		  ...
		  if(isset($sheet['cells'][$x][$y]) && in_array($cell, $luoghi)){
			   ...
			   if (!in_array($str, $corsi) && $cellPrec != "Dietistica") {
	...
	<div class="intro">
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Main -->
					<section class="sect">
					  <header class="section_header">Corso di Dietistica </header>
<h3>Definizione</h3>
<p>Persona con qualifica legalmente riconosciuta in nutrizione e dietistica che applica la scienza della nutrizione all’alimentazione
 e all’educazione di gruppi di persone e di individui sia in stato di salute, sia in stato di malattia.</p>
<h3>Funzioni</h3>
<ul>
	<li>Promozione, educazione, alla salute e prevenzione</li>
	<li>Sicurezza alimentare e ristorazione collettiva (certifcati HACCP, ecc…)</li>
	<li>Assistenza nutrizionale</li>
	<li>Organizzazione e gestione in ambito nutrizionale</li>
	<li>Formazione in ambito nutrizionale</li>
	<li>Ricerca in ambito nutrizionale</li>
</ul>
...
<ul>
	...
	<li>Azienda di ristorazione collettiva </li>
...
<button class="rounded" id="butt1" onclick="location.href='http://corsi.unibo.it/laurea/dietistica/Pagine/default.aspx'">Collegamento sito</button>
