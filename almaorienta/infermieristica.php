	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
	      $cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
 		  if(isset($sheet['cells'][$x][$y]) && $sheet['cells'][$x][2] == "Infermieristica"){
	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
		  ...
		  if(isset($sheet['cells'][$x][$y]) && in_array($cell, $luoghi)){
			   ...
			   if (!in_array($str, $corsi) && $cellPrec != "Infermieristica") {
...
<ul>
	...
	<li>IRCSS </li>
	<li>Case della salute</li>
	<li>Emergenza extra-ospedaliera</li>
	<li>Domicilio</li>
	<li>Ambulatori</li>
	<li>Consultori</li>
...
<button class="rounded" id="butt1" onclick="location.href='http://corsi.unibo.it/infermieristicabologna/Pagine/default.aspx'">Collegamento sito</button>
