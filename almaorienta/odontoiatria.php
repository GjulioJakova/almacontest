	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
	      $cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
 		  if(isset($sheet['cells'][$x][$y]) && $sheet['cells'][$x][2] == "Odontoiatria protesi dentaria"){
	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
		  ...
		  if(isset($sheet['cells'][$x][$y]) && in_array($cell, $luoghi)){
			   ...
			   if (!in_array($str, $corsi) && $cellPrec != "Odontoiatria protesi dentaria") {
	...
	<div class="intro">
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Main -->
					<section class="sect">
					  <header class="section_header">Corso di Odontoiatria protesi dentaria (Laurea Magistrale)</header>
<h3>Definizione</h3>
<p>L’odontoiatra svolge attività inerenti la prevenzione, diagnosi e terapia di malattie legate alla bocca.</p>
<h3>Funzioni</h3>
<ul>
	<li>Diagnostica e previene malattie ed anomalie congenite o acquisite della bocca, delle ossa mascellari, delle articolazioni
	temporo- mandibolari e dei relativi tessuti</li>
	<li>Esegue la riabilitazione odontoiatrica prescrivendo tutti i medicamenti e presidi necessari all’esercizio della professioni</li>
</ul>
 
 
<h3>Sbocchi lavorativi</h3>
<ul>
	<li>Libero professionista in studi associati o individuali</li>
	<li>Attività dirigenziale di primo e secondo livello presso USL</li>
</ul>
...
<button class="rounded" id="butt1" onclick="location.href='http://corsi.unibo.it/magistralecu/odontoiatria/Pagine/default.aspx'">Collegamento sito</button>
