	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
	      $cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
 		  if(isset($sheet['cells'][$x][$y]) && $sheet['cells'][$x][2] == "Tecnico della prevenzione nei luoghi di lavoro"){
	...
	while($x <= $sheet['numRows']){
		...
		while($y <= $sheet['numCols']){
		  ...
		  if(isset($sheet['cells'][$x][$y]) && in_array($cell, $luoghi)){
			   ...
			   if (!in_array($str, $corsi) && $cellPrec != "Tecnico della prevenzione nei luoghi di lavoro") {
	...
	<div class="intro">
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Main -->
					<section class="sect">
					  <header class="section_header">Corso di Tecnico della prevenzione nei luoghi di lavoro </header>
<h3>Definizione</h3>
<p>Professionista che è responsabile di tutte le attività di prevenzione, verifica e controllo in materia di sicurezza e igiene ambientale 
nei luoghi di vita e lavoro, di igiene di sanità pubblica e veterinaria.</p>
<h3>Funzioni</h3>
<ul>
	<li>Svolge compiti ispettivi e di vigilanza nel settore pubblico (ufficiale di polizia giudiziaria)</li>
	<li>Svolge attività istruttoria finalizzata al rilascio di autorizzazioni o di nulla osta tecnico-sanitari per attività soggette a controllo</li>
	<li>Propone misure preventive, organizza e attiva i processi di informazione e formazione aziendale</li>
</ul>
...
<ul>
	...
	<li>Dipartimenti di sanità pubblica </li>
	<li>Agenzie Regionali per la prevenzione e l’ambiente </li>
	<li>Servizi di protezione aziendali e industrie del settore alimentare</li>
</ul>
...
<button class="rounded" id="butt1" onclick="location.href='http://corsi.unibo.it/laurea/prevenzioneambientelavoro/Pagine/default.aspx'">Collegamento sito</button>
